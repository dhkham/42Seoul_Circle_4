...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/09 20:20:28 by chanwoki          #+#    #+#             */
/*   Updated: 2023/09/09 20:20:30 by chanwoki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void raycasting(t_info *info)
{
	...
	while (y < HEIGHT) // 천장, 바닥 색칠하기
	{
		...
		while (x < WIDTH)
		{
			info->buf[y][x] = rgb_to_int(info->config.floor_color[0], info->config.floor_color[1], info->config.floor_color[2]);	// floor
			info->buf[HEIGHT - y - 1][x] = rgb_to_int(info->config.ceiling_color[0], info->config.ceiling_color[1], info->config.ceiling_color[2]);	// ceiling
			x++;
		}
		y++;
	}
	for (int x = 0; x < WIDTH; x++)
	{
		...
		while (hit == 0) //DDA 알고리즘 : 광선이 부딪힐 때까지 계속 광선을 한 칸씩 이동
		{
			...
			if (info->config.map[mapX][mapY] != '0') 			//광선이 벽에 부딪혔는지 확인 (에러 지점: seg fault)
		...
		// 광선이 부딪힌 벽의 방향에 따라 사용할 텍스처를 결정
		// 방향에 대한 확인 필요
		int texNum;
		if (side == 0)   // If ray hit a wall in the x-direction
		{
			//printf("if side = 0\n");
			if (rayDirX > 0) // If ray is pointing to the right
				texNum = 1;	// S
			else
				texNum = 0; // N
		}
		else  // If ray hit a wall in the y-direction
		{	//printf("if side != 0\n");
			if (rayDirY > 0) // If ray is pointing downwards
				texNum = 3;	// E
			else
				texNum = 2; // W
		}
